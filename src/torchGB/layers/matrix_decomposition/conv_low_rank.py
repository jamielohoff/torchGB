...
# TODO fix the sizes of the predicted slices

def conv2d_low_rank_layer(param: Tensor, hidden_dim: int, gnet_batchsize: int):
    """
    ...
    # NOTE: this might need a different shape...
...
def init_conv2d_low_rank(pname: str, param: Tensor, hidden_dim: int,
                         gnet_batchsize: int):
    """
    ...
    """
    if "weight" in pname:
        return conv2d_low_rank_layer(param, hidden_dim, gnet_batchsize)

    else:
        return None


def build_conv2d_low_rank_output(name: str, param: Tensor, weights: Tensor,
