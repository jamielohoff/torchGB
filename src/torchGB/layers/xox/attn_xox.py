...
from .linear_xox import linear_xox_layer
from .model import XOXLayer
from ...utils import ceil, crop_matrix, build_matrix


def attn_xox_layer(param: Tensor, hidden_dim: int, gnet_batchsize: int):
    """
...
def init_attn_xox(pname: str, param: Tensor, hidden_dim: int,
                       gnet_batchsize: int):
    """
    ...
    if "in_proj_weight" in pname:
        return attn_xox_layer(param, hidden_dim, gnet_batchsize)
...
def build_attn_xox_output(name: str, param: Tensor, weights: Tensor,
